...
while not rospy.is_shutdown():
 if x >= pcx1 and  x <= pcx2 and y>= pcy1 and y <= pcy2:
    ...
    if right>1.0 and fro<1.7  and abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=0.5 :
    ...
    if abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=5:

                 col=2
                 print "speed reduced" 
                 speed.linear.x = abs(a1*x+b1*y-c1)
    ...
    if abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=1.0 :
         ...
         if abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=0.2 and fr<1:
             if col==2:
               speed.linear.x = 0.0
               speed.angular.z = 0.0  
               obs=0
    ...
    if abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=1.0 :
         print ("waiting for m line")
         if abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=0.2 and front<1.5:
                col=2
                print "col 1 detected" 
                #speed.linear.x = 0.5
                #speed.angular.z = -0.2
                sleep(3) 
         elif abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=0.5 and ri>2 and le>1.5 :
            if col==2:
               speed.linear.x = 0.0
               speed.angular.z = 0.0  
               obs=0
    ...
    if abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=1.0 :
         ...
         if right>1.0 and fro>1.0 and le <0.8  and abs(a1*x+b1*y-c1)>=0 and abs(a1*x+b1*y-c1)<=0.5 :
             if col==2:
               speed.linear.x = 0.0
               speed.angular.z = 0.0  
               obs=0
    ...
    if  x >= pcx1 and  x <= pcx2 and y>= pcy1 and y <= pcy2:
        ...
        speed.linear.x = 0.0
        speed.angular.z = -0.3 # taken minus to cheack
        print "rotate"
    else:
        if counter==1:
         speed.linear.x = 0.4
    ...
    if (fleft<1.2 and front<1.5 and fright<1.2 ) or (fleft>1.2 and front<1.5 and fright<1.2 ) or (fleft<1.2 and front<1.5 and fright>1.2 ) and obs==1:
        """
         while fleft > 1.5  and front > 1.5 and fl < 1:
             speed.linear.x = 0.5
             speed.angular.z = 0.0
    ...
    if fleft<1.2 and front>1.2 and fright>1.2 and obs==1: #made fright 1.3 from 1.0
        speed.linear.x = 0.5
        speed.angular.z = 0.0
        al=1
        print "following"

    if fleft>1.3 and front>1.0 and fright>1.0 and obs==1 and al==1:
        """speed.linear.x=0.8
        w=0
        while w==100:
           break 
    ...
    if le<0.5 and fr>1.0 and fright>1.0 and obs==1 and al==1:
        """speed.linear.x=0.8
        w=0
        while w==100:
           break 
    ...
    """
    if front<2  and  abs((a1*x+b1*y)-c1) : #Near goal
